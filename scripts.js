...
document.addEventListener("DOMContentLoaded", () => {
    const titleBase = "MILY\""; // Base du titre
    ...
    let initialSection = document.getElementById(initialTargetId);

    function updateActiveLinks(targetId) {
        document.querySelectorAll("nav a, #txth1 a").forEach(link => {
    ...
    if (initialSection) {
        document.querySelectorAll("section").forEach(section => {
            ...
            section.style.transition = "opacity 0.1s ease, filter 0.1s ease";
        });

        initialSection.style.display = "flex";
        setTimeout(() => {
            initialSection.style.opacity = "1";
            initialSection.style.filter = "blur(0)";
        }, 200);
        
        document.title = `${initialTargetId.charAt(0).toUpperCase() + initialTargetId.slice(1)} - ${titleBase}`;
    }

    updateActiveLinks(initialTargetId);

    document.querySelectorAll("nav a, #txth1 a, a[href^='#']").forEach(link => {
        link.addEventListener("click", function(event) {
            ...
            if (targetSection) {
                ...
                setTimeout(() => {
                    ...
                    targetSection.style.filter = "blur(10px)";
                    targetSection.style.transition = "opacity 0.1s ease, filter 1s ease";
                    ...
                    document.title = `${targetId.charAt(0).toUpperCase() + targetId.slice(1)} - ${titleBase}`;
                }, 400);
            }
        });
    });
    ...
    window.addEventListener("popstate", () => {
        let targetId = window.location.hash.substring(1) || "homepage";
        let targetSection = document.getElementById(targetId);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const sidebarLinks = sidebar.querySelectorAll("a"); // Sélectionne tous les liens dans la sidebar

    // Vérifier l'existence des éléments avant d'ajouter les écouteurs
    if (!menuToggle || !sidebar || !menuClose) return;
    ...
    // Fermer la sidebar si l'utilisateur clique en dehors
    ...
    // Empêcher la fermeture si on clique à l'intérieur de la sidebar
...
document.addEventListener("DOMContentLoaded", function () {
    let slideIndex = 1;
    let timer;
    const slideDuration = 5000; // Durée fixe pour chaque slide
    ...
    function plusSlides(n) {
        ...
        resetAutoSlide(); // Réinitialise le diaporama après un clic manuel
    }

    // Déclarer currentSlide globalement pour onclick
    window.currentSlide = function(n) {
        slideIndex = n;
        showSlides(slideIndex);
        stopAutoSlide(); // Arrêter l'animation automatique après un clic sur un dot
    };
    ...
    dots.forEach((dot, index) => {
        ...
        // Arrêter l'animation si le curseur reste sur le dot
        dot.addEventListener("mouseover", stopAutoSlide);
        dot.addEventListener("mouseout", startAutoSlide);
    });

    function startAutoSlide() {
        timer = setInterval(() => plusSlides(1), slideDuration);
    }
    ...
    function resetAutoSlide() {
        stopAutoSlide();
        startAutoSlide(); // Redémarre le diaporama avec le bon intervalle
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showSlides(n) {
    const slides = document.querySelectorAll(".prtflitem > div"); // Récupère les prtfl1, prtfl2, etc.
    const dots = document.querySelectorAll(".carousel-indicator"); // Récupère les dots
    ...
    // Masquer toutes les slides et désactiver les dots
  ...
  // Fonction pour sélectionner une slide spécifique
  ...
  // Gestion automatique (diaporama)
  let timer;
  function startAutoSlide() {
    timer = setInterval(() => plusSlides(1), 3000); // Change chaque 10 secondes
  }
  function stopAutoSlide() {
    clearInterval(timer);
  }

  // Ajouter des événements au survol
  const portfoli = document.querySelector(".portfoli");
  if (portfoli) {
    portfoli.addEventListener("mouseover", stopAutoSlide);
    portfoli.addEventListener("mouseout", startAutoSlide);
  }

  startAutoSlide(); // Démarrer le diaporama automatiquement
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let isZoomed = false, translateX = 0, translateY = 0, scale = 1;
    ...
    link.addEventListener("click", e => {
        ...
        lightboxDesc.textContent = link.getAttribute("data-description") || "Pas de description";

        lightbox.style.display = "flex";
        document.body.classList.add("lightbox-open");
        window.scrollTo(0, 0);

        lightboxImg.onload = () => updateDescriptionWidth(); // Mise à jour immédiate après le chargement
        updateDescriptionWidth(); // Ajustement immédiat, évite le délai
    });
});
    ...
    // 🎯 Quitter la lightbox avec la touche Échap
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape") closeLightbox();
    });

    // 🔍 Zoom dynamique sur la partie cliquée
    lightboxImg.addEventListener("dblclick", (e) => {
        if (!isZoomed) {
            ...
            translateX = (rect.width / 2 - offsetX) * 0.5;
            translateY = (rect.height / 2 - offsetY) * 0.5;
            scale = 2;
    ...
    function onMouseMove(e) {
        ...
        lightboxImg.style.transform = `translate(${translateX + moveX}px, ${translateY + moveY}px) scale(${scale})`;
    }
...
lightboxImg.addEventListener("touchstart", (e) => {
    ...
    document.addEventListener("touchmove", onTouchMove, { passive: true }); // ✅ Améliore la fluidité du scroll
    document.addEventListener("touchend", onTouchEnd, { passive: true }); // ✅ Corrige le blocage
}, { passive: true }); // ✅ Ajout de `passive: true` à touchstart
    ...
    // 📏 Ajuster largeur de la description
    function updateDescriptionWidth() {
        lightboxDesc.style.width = `${lightboxImg.getBoundingClientRect().width}px`;
    }
...
document.getElementById("contact-form").addEventListener("submit", function(event) {
    ...
    fetch(this.action, {
        method: "POST",
        headers: { "Content-Type": "application/json" }, // Ajout de l'en-tête JSON
...
// page loader
document.addEventListener('DOMContentLoaded', function () {
    const loader = document.getElementById('page-loader'); // Loader

    window.onload = function () {
        if (loader) {
            loader.style.display = 'none'; // ✅ Masque le loader dès que la page est chargée
        }
    };
});
...
particlesJS('particle-container', {
    particles: {
        number: {
            ...
            density: {
                enable: true,
                value_area: 600 // Densité des particules
            }
        },
        color: {
        ...
        move: {
            ...
            random: false,
            straight: false,
            out_mode: "out", // Les particules sortent du conteneur
            bounce: false,
            attract: {
                enable: false,
                rotateX: 600,
    ...
    interactivity: {
        ...
        events: {
            onhover: {
                enable: true,
                mode: "repulse" // Les particules se repoussent quand la souris passe dessus
            },
            onclick: {
                enable: true,
                mode: "push" // Ajoute des particules lorsqu'on clique
            },
            resize: true
        },
        modes: {
...
// HOME
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top'); // Bouton "Back-to-Top"
    const targetElement = document.querySelector('.secondcontent'); // L'élément cible
    
    // Observer si l'élément est visible
    ...
    // Action au clic pour revenir en haut
    backToTopButton.addEventListener('click', function () {
        // Revenir directement en haut
        document.documentElement.scrollTop = 0; // Pour la plupart des navigateurs modernes
        document.body.scrollTop = 0; // Alternative pour certains anciens navigateurs
    });
});
// Bouton "Back-to-Top" 
// ABOUT
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top1'); // Bouton "Back-to-Top"
    const targetElement = document.querySelector('.servAreaparagraphe4A1'); // L'élément cible
...
// SERVICES
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top2'); // Bouton "Back-to-Top"
    const targetElement = document.querySelector('.services4'); // L'élément cible
...
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top3'); // Bouton "Back-to-Top"
    const targetElement = document.querySelector('.resume4serv'); // L'élément cible
...
// PORTFOLIO
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top4'); // Bouton "Back-to-Top"
    const targetElement = document.querySelector('.prtfl1a'); // L'élément cible
...
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top5'); // Bouton "Back-to-Top"
    const targetElement = document.querySelector('.prtfl5a'); // L'élément cible
...
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top6'); // Bouton "Back-to-Top"
    const targetElement = document.querySelector('.prtfl7a'); // L'élément cible
...
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top7');
    const targetElements = document.querySelectorAll('.prtfl4a'); 
    ...
    const observer = new IntersectionObserver(function (entries) {
        ...
        backToTopButton.style.display = visibleElements.size > 0 ? 'block' : 'none';
    });
...
document.addEventListener('DOMContentLoaded', function () {
    const backToTopButton = document.getElementById('back-to-top8'); // Bouton "Back-to-Top"
    const targetElement = document.querySelector('.portfolioinitialz'); // L'élément cible
...
!function(D, u) {
    ...
    function emojiSafeSplit(D, u, F) {
        ...
        for (var C, E, e = [], t = D.length, n = 0; n < t; n++)
            (55296 <= (E = D.charAt(n)).charCodeAt(0) && E.charCodeAt(0) <= 56319 || 65024 <= D.charCodeAt(n + 1) && D.charCodeAt(n + 1) <= 65039) && (C = ((D.substr(n, 12).split(i) || [])[1] || "").length || 2,
            E = D.substr(n, C),
            n += C - (e.emoji = 1)),
            e.push(">" === E ? "&gt;" : "<" === E ? "&lt;" : E);
        return e
    ...
    var u, l, F = {
        ...
        init: function init(D, u, F) {
            var C, E, e, t, n, B, i, A, r = D.nodeName.toUpperCase(), s = this;
            if (s.svg = D.getBBox && ("TEXT" === r || "TSPAN" === r),
        ...
        render: function render(D, u) {
            1 < D ? D = 1 : D < 0 && (D = 0),
            u.from && (D = 1 - D);
            var F, C, E, e = u.text, t = u.hasClass, n = u.newClass, B = u.oldClass, i = u.delimiter, A = u.target, r = u.fillChar, s = u.original, l = e.length, o = D * l + .5 | 0;
            E = t ? (C = B && o !== l,
            ((F = n && o) ? "<span class='" + n + "'>" : "") + e.slice(0, o).join(i) + (F ? "</span>" : "") + (C ? "<span class='" + B + "'>" : "") + i + s.slice(o).join(i) + (C ? "</span>" : "")) : e.slice(0, o).join(i) + i + s.slice(o).join(i),
...
const couleurs = ["red", "blue", "green", "black"];
...
setTimeout(function () {
    gsap.to(texteElement, {
        duration: 2, // Durée de l'écriture
        text: 'MILY"- "Make innovations lead you"', // Texte affiché
...
}, 2700); // Temps d'attente de 3 secondes
    ...
    B.extend({
      ...
      , Ke = B.extend({
        options: {
            ...
            prefix: '<a href="https://leafletjs.com" title="A JavaScript library for interactive maps">' + (b.inlineSvg ? '<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="12" height="8" viewBox="0 0 12 8" class="leaflet-attribution-flag"><path fill="#4C7BE1" d="M0 0h12v4H0z"/><path fill="#FFD500" d="M0 4h12v3H0z"/><path fill="#E0BC00" d="M0 7h12v1H0z"/></svg> ' : "") + "</a>"
        },
        initialize: function(t) {
            c(this, t),
            this._attributions = {}
...
//script interactive map zone point
// script map zone
document.addEventListener("DOMContentLoaded", function () {
    // Définir les couches OpenStreetMap et Satellite
    ...
    // Initialisation de la carte avec les paramètres par défaut
    var mapportfolio = L.map('mapportfolio', {
        center: [-18.766947, 48.869107], // Coordonnées pour madagascar
        zoom: 5.33,
        layers: [osmLayer], // Couche par défaut
...
if (window.innerWidth < 768) { // Vérifie si l'écran est de petite taille (mobile)
    mapportfolio.dragging.disable();
}

// Activer le déplacement après un clic sur la carte
mapportfolio.on('click', function () {
    mapportfolio.dragging.enable();
});

// Désactiver à nouveau lorsqu'on quitte la carte
mapportfolio.on('mouseout', function () {
...
mapportfolio.getContainer().addEventListener('touchmove', function () {
    if (!hasInteracted && !tooltip.parentNode) {
        document.getElementById('mapportfolio').appendChild(tooltip);
    ...
    // Couches de base pour le contrôle
    var baseMaps = {
        "Carte Standard": osmLayer,
        "Satellite": satelliteLayer
    };
    L.control.layers(baseMaps).addTo(mapportfolio);

fetch('LROYRSF.geojson')
    .then(response => response.json())
    .then(data => {
        let regionSofiaLayer = L.geoJSON(data, {
            style: function (feature) {
                return {
                    color: '#008000', // Contour vert
                    weight: 2, // Épaisseur du contour
                    fillColor: '#66CC66', // Remplissage vert clair
                    fillOpacity: 0.1 // Opacité réduite
                };
            },
            onEachFeature: function (feature, layer) {
                layer.bindPopup(feature.properties.nom || 'Région Sofia | Votre projet');
            }
        }).addTo(mapportfolio);

        // Récupérer la position du haut du polygone
        let topPoint = regionSofiaLayer.getBounds().getNorthEast();

        // Déplacement manuel : réduire latitude (descendre) et longitude (pousser à gauche)
        let adjustedPoint = [topPoint.lat - 0.52, topPoint.lng - 1.02]; 

        L.popup()
            .setLatLng(adjustedPoint)
            .setContent('Région Sofia | Votre projet')
            .openOn(mapportfolio);
    })
    ...
    // Gestion des interactions et de l'infobulle
    let hasInteracted = false; // Variable pour suivre si l'utilisateur a interagi
    const tooltip = L.DomUtil.create('div', 'map-tooltip');
    tooltip.innerHTML = "Cliquez pour activer la carte.";
    ...
    // Activer le zoom au défilement après un clic
    mapportfolio.on('click', function () {
        if (!hasInteracted) {
            hasInteracted = true;
            if (tooltip.parentNode) {
        ...
        mapportfolio.scrollWheelZoom.enable();
    });

    // Afficher une infobulle si la molette de la souris est utilisée avant interaction
    mapportfolio.getContainer().addEventListener('wheel', function () {
        if (!hasInteracted && !tooltip.parentNode) {
            document.getElementById('mapportfolio').appendChild(tooltip);
            setTimeout(() => {
                if (tooltip.parentNode) {
    ...
    // Réinitialiser l'état lorsque la souris quitte la carte
    mapportfolio.on('mouseout', function () {
        mapportfolio.scrollWheelZoom.disable(); // Désactiver le zoom au défilement
        hasInteracted = false; // Réinitialiser l'état d'interaction
        document.getElementById('mapportfolio').style.cursor = 'default'; // Revenir au curseur par défaut
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Initialisation de la carte avec les paramètres par défaut
    var intermap1 = L.map('intermap1', {
        center: [-18.8392, 47.5079], // Coordonnées pour madagascar
        zoom: 12,
        layers: [satelliteLayer], // Couche par défaut
        scrollWheelZoom: false // Désactiver le zoom au défilement par défaut
    });

    // Désactiver le glissement de la carte sur mobile
if (window.innerWidth < 768) { // Vérifie si l'écran est de petite taille (mobile)
    intermap1.dragging.disable();
}

// Activer le déplacement après un clic sur la carte
intermap1.on('click', function () {
    intermap1.dragging.enable();
});

// Désactiver à nouveau lorsqu'on quitte la carte
intermap1.on('mouseout', function () {
    if (window.innerWidth < 768) {
        intermap1.dragging.disable();
    }
});
...
// Afficher l'infobulle si l'utilisateur essaie de glisser sur mobile sans avoir interagi
intermap1.getContainer().addEventListener('touchmove', function () {
    if (!hasInteracted && !tooltip.parentNode) {
        document.getElementById('intermap1').appendChild(tooltip);
    ...
    L.control.layers(baseMaps).addTo(intermap1);

    // Ajouter un marqueur avec une popup
L.marker([-18.8792, 47.5079]).addTo(intermap1)
    .bindPopup('Votre bureau ou autres!', { autoClose: false })
    .openPopup();

L.marker([-18.8500, 47.5200]).addTo(intermap1)
    .bindPopup('Ce que vous voulez montrer avec une petite description comme ça!')
    .openPopup();

// Ajouter un contrôle d'échelle à la carte
L.control.scale({
    position: 'bottomleft', // Position de l'échelle (par défaut : 'bottomleft')
    metric: true, // Afficher les unités métriques (mètres/kilomètres)
    imperial: false // Ne pas afficher les unités impériales
}).addTo(intermap1);
    
    // Style initial du curseur : par défaut
        document.getElementById('intermap1').style.cursor = 'default';
    ...
    intermap1.on('click', function () {
        if (!hasInteracted) {
            ...
            document.getElementById('intermap1').style.cursor = 'grab';
        }
        intermap1.scrollWheelZoom.enable();
    });

    // Afficher une infobulle si la molette de la souris est utilisée avant interaction
    intermap1.getContainer().addEventListener('wheel', function () {
        if (!hasInteracted && !tooltip.parentNode) {
            document.getElementById('intermap1').appendChild(tooltip);
            setTimeout(() => {
                if (tooltip.parentNode) {
    ...
    // Réinitialiser l'état lorsque la souris quitte la carte
    intermap1.on('mouseout', function () {
        intermap1.scrollWheelZoom.disable(); // Désactiver le zoom au défilement
        hasInteracted = false; // Réinitialiser l'état d'interaction
        document.getElementById('intermap1').style.cursor = 'default'; // Revenir au curseur par défaut
    });
});
